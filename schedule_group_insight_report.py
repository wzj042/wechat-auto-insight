#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""注册 Windows 任务计划，每日定时运行根目录 `group_insight_report.py`。"""
...
def resolve_executable(value: str) -> str:
    ...
    resolved = shutil.which(value) if os.path.sep not in value and (os.path.altsep is None or os.path.altsep not in value) else None
    if resolved:
        return resolved
...
def register_task(
    ...
    *,
    ...
    task.RegistrationInfo.Description = f"Daily run of {script_path.name}"
    ...
    action.Arguments = build_arguments(script_path, task_args)
...
def main() -> None:
    ...
    command_line = build_arguments(script_path, args.args)
